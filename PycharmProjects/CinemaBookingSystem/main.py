    ...
    try:
        ...
        if cursor:
            try:
                ...
                # Simplified Booking Details Fetch
                q_det = f"""
                    SELECT b.seat_id 
                    FROM bookings b 
                    WHERE b.customer_id = {PH}
                """
                cursor.execute(q_det, (customer_id,))
                seats_data = cursor.fetchall()
                if seats_data:
                    seat_id_val = ", ".join([str(s[0]) for s in seats_data])
                    seats_str = f"{len(seats_data)} Reserved Seat(s)"

            except Exception as db_err:
                print(f"[PDF DB Warning] Falling back to default values: {db_err}")

        # Construct ReportLab Document
        ...
        label_style = ParagraphStyle('Label', fontName='Helvetica-Bold', fontSize=8, textColor=colors.HexColor("#666666"), leading=10)
        val_style = ParagraphStyle('Value', fontName='Helvetica-Bold', fontSize=10, textColor=colors.HexColor("#222222"), leading=13)
        ...
        header_table = Table([[Paragraph("🍿", ParagraphStyle('Logo', fontSize=24)), header_text]], colWidths=[0.9 * inch, 5.3 * inch])
        ...
        grid_data = [
            [Paragraph("CUSTOMER NAME", label_style), Paragraph("BOOKING REF", label_style)],
            [Paragraph(customer_name, val_style), Paragraph(f"#BK-{booking_id}", val_style)],
            [Spacer(1, 6), Spacer(1, 6)],
            [Paragraph("SEAT PASS(ES)", label_style), Paragraph("PAYMENT METHOD", label_style)],
            [Paragraph(seats_str, val_style), Paragraph(str(payment_method), val_style)],
            [Spacer(1, 6), Spacer(1, 6)],
            [Paragraph("TOTAL PAID", label_style), Paragraph("STATUS", label_style)],
            [Paragraph(f"Rs. {float(total_amount):.2f}", val_style), Paragraph("CONFIRMED", val_style)]
        ]

        ticket_body = Table(grid_data, colWidths=[3.1 * inch, 3.1 * inch])
